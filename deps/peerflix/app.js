  ...
  if (argv.list) {
    var onready = function() {
      engine.files.forEach(function(file, i, files) {
        clivas.line('["index": "'+i+'", "path": "'+file.name+'", "size": "'+file.length+'", "readable": "'+bytes(file.length)+'"]')
      })
      process.exit(0)
    }
...
//      else clivas.line('["link": "'+href+'"]')
	  clivas.line('')
	  clivas.line('["link": "'+href+'"]')


      clivas.line('["filename": "'+filename+'", "size": "'+filelength+'", "readable": "'+bytes(filelength)+'"]')
      clivas.line('["path": "' + engine.path.replace(/\\/g, "\\\\") + '"]')
      clivas.line('["downloaded": "'+swarm.downloaded+'", "readable": "'+bytes(swarm.downloaded)+'"]')
	  clivas.line('["uploaded": "'+swarm.uploaded+'", "readable": "'+bytes(swarm.uploaded)+'"]')
	  clivas.line('["runtime": "'+runtime+'"]')
      clivas.line('["peerqueue": "'+swarm.queued+'"]')
...
//        clivas.line('{25+magenta:'+wire.peerAddress+'} {10:'+bytes(wire.downloaded)+'} {10+cyan:'+bytes(wire.downloadSpeed())+'/s} {15+grey:'+tags.join(', ')+'}   ')
  ...
  var onmagnet = function() {
    clivas.line('["streams": "'+engine.swarm.wires.length+'"]')
  }
