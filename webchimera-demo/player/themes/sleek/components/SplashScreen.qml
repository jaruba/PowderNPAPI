...
Rectangle {
	...
	Rectangle {
		...
		Rectangle {
			...
			Text {
				id: loadingFont
				visible: multiscreen == 1 ? fullscreen ? vlcPlayer.state == 1 || vlcPlayer.state == 0 ? true : buffering > 0 && buffering < 100 ? true : false : false : vlcPlayer.state == 1 || vlcPlayer.state == 0 ? true : buffering > 0 && buffering < 100 ? true : false // Required for Multiscreen
				anchors.top: parent.top
				anchors.topMargin: 80
				anchors.horizontalCenter: parent.horizontalCenter
				text: vlcPlayer.state == 0 ? peers > 0 ? "Connecting to "+peers+" streams" : "Fetching Data" : "Initializing Buffer"
